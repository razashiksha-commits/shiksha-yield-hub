...
def gsc_query_payload(page_url: str, start_date: str, end_date: str, row_limit: int = 25) -> dict:
    """Payload for fetching the top search queries that drive traffic to ONE page."""
    return {
        "startDate": start_date,
        "endDate": end_date,
        "dimensions": ["query"],
...
def ga4_payload(start_date: str, end_date: str, event_name: str) -> dict:
    """Conversions payload: counts a specific named event per landing page."""
    return {
...
def ga4_sessions_payload(start_date: str, end_date: str) -> dict:
    """Organic sessions payload: total sessions per landing page, no event filter."""
    return {
        "dateRanges": [{"startDate": start_date, "endDate": end_date}],
        "dimensions": [{"name": "date"}, {"name": "landingPagePlusQueryString"}],
...
def _norm_url(u: str) -> str:
    """Normalise a URL for matching: strip query string, strip trailing slash.
    Stripping the query string is what lets '/news/x' and '/news/x?utm=fb'
    match the same GSC row. NOTE: because of this, GA4 frequently returns
    several rows that all normalise to the same URL (e.g. utm variants) -
    see _collapse() below, which is the fix that prevents that from
    inflating totals when frames are merged together."""
...
def _parse_ga4(ga4_json: dict, site_url: str, value_col: str) -> pd.DataFrame:
    """Shared GA4 parser. value_col names the output metric column
    ('Conversions' or 'Sessions') so both report types share one code path."""
    rows = []
    ...
    for row in ga4_json.get("rows", []):
        ...
        rows.append({
            "Date": date,
            "URL": _norm_url(full),
            value_col: int(float(mets[0]["value"])),
        })
    return pd.DataFrame(rows)


def parse_ga4(ga4_json: dict, site_url: str) -> pd.DataFrame:
    """Turn a raw GA4 runReport response (conversions) into a tidy DataFrame."""
...
def _collapse(df: pd.DataFrame, sum_cols, mean_cols=None) -> pd.DataFrame:
    ...
    that normalise to the same URL (see _norm_url) create duplicate keys, and
    merging GSC x GA4-conversions x GA4-sessions becomes a cartesian product
    that silently inflates every total (impressions, clicks, conversions,
    sessions all multiply together). This must run on every input BEFORE
    any merge happens."""
...
def build_yield_table(gsc_df: pd.DataFrame, ga4_df: pd.DataFrame,
    ...
    Each input is first collapsed to one row per (Date, URL) via _collapse()
    so the joins are strictly 1-to-1 and metrics are never double-counted,
    even when GA4 returns multiple query-string variants of the same page."""
...
def find_opportunities(df: pd.DataFrame, min_impressions: int = 50) -> pd.DataFrame:
    ...
    SORT FIX: ties in Potential_Gain are broken by Clicks then Impressions.
    This matters constantly, not rarely - the most common trigger is a date
    range where site-wide conversions are 0 (every page ties at gain=0), but
    it also happens any time several pages happen to land on the same
    recoverable-conversion estimate. Without this tie-break, pandas falls
    back to arbitrary groupby("URL") hash order, which has zero relationship
    to traffic size - meaning your single biggest page (by impressions and
    clicks) can silently fall below a head(N) display cutoff while a much
    smaller page wins the tie and gets shown instead. Always break ties by
    real traffic so the highest-impact pages are guaranteed to surface."""
    if df.empty:
        return pd.DataFrame()
    ...
    # Tie-break by Clicks then Impressions so high-traffic pages can never
    # be lost to arbitrary ordering when Potential_Gain values tie.
